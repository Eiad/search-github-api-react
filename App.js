    ...
    if (this.state.selectionz === "users") {
      ...
      <div className="main-cont">
         <div className="container">
            <h2 className="center">Search Users / Repositories / Issues From github</h2>
      ...
      let reposlist = data.items.map(reposinfoz => {
        return (
          ...
          <p className="left repo-desc">
             ...
             <br />Desc: {reposinfoz.description}
             <br />By: {reposinfoz.owner.login}
             <br />Last Update: {reposinfoz.updated_at}
          </p>
          </div>
          </div>
        );
    ...
    if (data.message === "Not Found")
      ...
      let reposlist = data.items.map(issuesinfoz => {
        return (
          ...
          <div className="single-block">
          <p className="left repo-desc">
             <a href={issuesinfoz.html_url} >Issue in Repo: {issuesinfoz.name}</a>
             <br />Issue events: {issuesinfoz.issue_events_url}
             <br />Issues URL: {issuesinfoz.issues_url}
             <br />Open Issues: {issuesinfoz.open_issues}
          </p>
          </div>
          </div>
        );
